...
See each wrapper directory README for instructions on building and installing each.

Furthermore, there is an interface library target that installs the header file. This target can also be digested by other CMake-based projects using the `add_subdirectory` command. Enable it like this:

```
mkdir buildIL
cd buildIL
cmake .. -DIF97_CMAKE_MODULE=ON
```
